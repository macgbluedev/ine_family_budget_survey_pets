...
# Extrae los gastos en mascotas y la clasificación de hogares a partir de los
# microdatos EPF (INE), para un año arbitrario.
#
# Uso:
#   Rscript ExtractResultFromMicrodataEpf.R <input_dir> <output_dir>
#
# Ejemplos:
#   Rscript ExtractResultFromMicrodataEpf.R Inputs/2024 Outputs/2024
#   Rscript ExtractResultFromMicrodataEpf.R Inputs/2025 Outputs/2025
#
# <input_dir>/R/ debe contener EPFgastos_<year>.RData y EPFhogar_<year>.RData
# (los nombres de las variables guardadas dentro pueden ser Microdatos/Metadatos
# o df_micro/df_meta según el año; ambos se soportan).
#
# Genera en <output_dir>/:
#   - EPFOnlyPetsResult.xlsx             (detalle de gasto por hogar x código)
...
# Localiza el único fichero <prefix>_*.RData dentro de <dir> (p.ej. prefix="EPFgastos").
find_rdata <- function(dir, prefix) {
    files <- list.files(dir, pattern = paste0("^", prefix, "_.*\\.RData$"), full.names = TRUE)
    if (length(files) != 1) {
        stop(sprintf("Se esperaba exactamente 1 fichero '%s_*.RData' en %s, encontrados %d",
                      prefix, dir, length(files)))
    }
    files[1]
}

# Carga un .RData y devuelve el primer data.frame conocido, sea cual sea el
# nombre con el que se guardó (Microdatos/Metadatos en 2024, df_micro/df_meta en 2025).
load_microdatos <- function(path) {
    e <- new.env()
    load(path, envir = e)
    for (name in c("Microdatos", "df_micro")) {
        if (exists(name, envir = e, inherits = FALSE)) {
            return(get(name, envir = e))
        }
    }
    stop(sprintf("Ninguna de las variables esperadas (Microdatos, df_micro) se encontró en %s. Disponibles: %s",
                 path, paste(ls(e), collapse = ", ")))
}

args <- commandArgs(trailingOnly = TRUE)
if (length(args) != 2) {
    stop("Uso: Rscript ExtractResultFromMicrodataEpf.R <input_dir> <output_dir>\n",
         "Ejemplo: Rscript ExtractResultFromMicrodataEpf.R Inputs/2024 Outputs/2024")
}
input_dir <- args[1]
output_dir <- args[2]

pathGastos <- find_rdata(file.path(input_dir, "R"), "EPFgastos")
pathHogar <- find_rdata(file.path(input_dir, "R"), "EPFhogar")

Gastos <- load_microdatos(pathGastos)
Hogar <- load_microdatos(pathHogar)

GastosOnlyPets <- subset(Gastos, CODIGO %in% c('09321','09322','09450'))
...
write.xlsx2 (GastoOnlyPetsHogar, file = file.path(output_dir, "EPFOnlyPetsResult.xlsx"), sheetName = "Result",col.names = TRUE, row.names = TRUE, append = FALSE)
...
write.xlsx2 (HogarFilteredByExpenseCode, file = file.path(output_dir, "EPFOnlyPetsProportionResult.xlsx"), sheetName = "Result",col.names = TRUE, row.names = TRUE, append = FALSE)

cat(sprintf("OK: escritos %s/EPFOnlyPetsResult.xlsx y %s/EPFOnlyPetsProportionResult.xlsx\n", output_dir, output_dir))
